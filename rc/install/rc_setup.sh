...
case \$- in *i*)
    ...
    _rc_reload() { rm -rf ~/rc; git clone https://github.com/qeatzy/rc ~/rc; }
esac
" >> ~/.bashrc;
fi
...
case \$- in *i*)
    ...
    _rc_reload() { rm -rf ~/rc; git clone https://github.com/qeatzy/rc ~/rc; bash ~/rc/rc/install/rc_setup.sh; }
esac
" >> ~/.zshrc;
fi

if ! grep "\(so\|source\) [-~/._A-Za-z0-9 ]*/${rc_root##*/}/\.vimrc" ~/.vimrc &>/dev/null; then echo "so $rc_root/.vimrc" >> ~/.vimrc; fi
