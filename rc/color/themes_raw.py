...
def cli_number_override(args):
    ...
    if m:
        ...
        set_theme((i,value) for i in range(start,stop))
    else:
        return args
...
def cli_list(args):
    if 'list'.startswith(args[0]):
        print("list all supported themes.")
        print("\n".join(_themes))
    else:
        return args
...
def cli_show(args):
    def show_color(nr,ncol=8):
        for start in range(0,len(nr),ncol):
            print(''.join('\033[38;5;{}m {:3d}'.format(i,i) for i in
                nr[start:start+ncol]))
...
def cli_show_rgb(args):
    if args[0] == 'r' and len(args) > 1:
        ...
        print(' '.join('\033[38;2;{};{};{}mcolor\033[0m'.format(*i)
            for i in v))
    else:
        return args
...
if __name__ == "__main__":
    ...
    #     print("not tty",end='')
    cli()
    # TODO: chained command, first set, then overridded. priority: low.
    # st()
