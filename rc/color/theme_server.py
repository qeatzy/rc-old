...
# https://github.com/Mayccoll/Gogh/tree/master/themes
    ...
    def _intermediate(func):
        os.makedirs("mkdir -p '{}/themes'".format(dir), exist_ok=True)
        # os.system("mkdir -p '{}/themes'".format(dir))
        # print("mkdir -p '{}/themes'".format(dir))
...
def parse_spec(specs):
    """ specs: iterable of (color_name, color) pair """
    # theme_name = None
    ret = []
    _d = {}
...
# temporary fix, add pull request of new theme, download manually first
_names = ['palenight']
def fetch_theme(name):  # override
    with open(os.path.join(_fetch_url.dir, 'themes', name+'.sh')) as f:
        return f.read()
try:
    for name in _names:
        theme_name, theme = get_theme(name)
